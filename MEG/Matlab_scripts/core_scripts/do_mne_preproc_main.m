...
%   Fahimeh Mamashli January 08, 2014
...
%    2) cfg.do_erm_filtering_mne=[];

%    3) cfg.do_erm_noise_covariance_mne=[];
    
%    4) cfg.do_mne_preproc_filtered=[];
...
%    5) cfg.do_eve_noise_covariance_mne=[];
% if you use erm data for noise covariance, then this opetion will be
% automatically off.

%    6) cfg.do_mne_preproc_grand_average=[];
...
if ~isfield (cfg,'current')
    ...
    if ~isfield(cfg,'do_mne_preproc_filtered')
     ...
     if ~isfield(cfg,'do_eve_noise_covariance_mne')
        cfg.do_eve_noise_covariance=1;
        fprintf('Performing eve_noise_covariance  %s\n',subj);
    else
        cfg.do_eve_noise_covariance=0;
        fprintf('Not Performing eve_noise_covariance  %s\n',subj);
    end
    ...
    if ~isfield(cfg,'do_mne_preproc_grand_average')
        ...
        fprintf('Not Performing Grand average, %s\n',subj);
    end
    
    % either noise covariance computed from erm or baseline data
    if cfg.do_erm_noise_covariance
        cfg.do_eve_noise_covariance=0;
        fprintf('You perform erm noise covariance, so eve noise covariance will not be performed  %s\n',subj);
    elseif cfg.do_eve_noise_covariance
        cfg.do_erm_noise_covariance=0;
        fprintf('You perform eve noise covariance, so erm noise covariance will not be performed  %s\n',subj);
    end
...
if (cfg.do_eve_noise_covariance)
    try
        fprintf('Starting eve_noise_covariance %s\n',subj);    
        [cfg]=do_eve_noise_covariance(subj,visitNo,run,cfg);
        fprintf('Complete ! %s\n',subj);

    catch ME
        cfg.current=ME.stack.name;
        filename=strcat(subj,'_do_eve_noise_covariance_error_cfg');
        save(filename,'cfg','visitNo','run','erm_run','subj','ME'); 
