function [data,times,ch_names] = mne_read_epochs(fname,event,eventname,tmin,tmax)
%
...
%
%   $Header: /space/orsay/8/users/msh/CVS/CVS-MSH/MNE/mne_matlab/mne_ex_read_epochs.m,v 1.7 2009/03/21 00:17:35 msh Exp $
%   $Log: mne_ex_read_epochs.m,v $
...
if count > 0
   ...
   warning(me,'No desired events found.');
end
...
for p = 1:count
    ...
    event_samp = double(events(selected(p),1));
    from = int32(event_samp + tmin*raw.info.sfreq);
    to   = int32(event_samp + tmax*raw.info.sfreq);
  %  try
        if p == 1
        ...
        if size(epoch,2)<abs(to-from)
        %if size(epoch,2)<ceil(to-from)
            continue
        else
        data(:,:,p) = epoch;
        end
