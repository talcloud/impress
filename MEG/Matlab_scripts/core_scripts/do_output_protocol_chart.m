function [cfgsheet,spreadsheet_start]=do_output_protocol_chart(masterroot,subj,visitNo,newrun,cfgsheet,cfg,spreadsheet_start,filenamecode)

addpath('/autofs/cluster/transcend/scripts/MEG/Matlab_scripts/core_scripts/Private_spreadsheet');
...
data_subjdir=[masterroot '/' subj '/' num2str(visitNo) '/'];
...
while(A==0)
    ...
    elseif filenamecode==4,
               filename=strcat(masterroot,'/',subj,'/',num2str(visitNo),'/',subj,'_epochMEG_main_cfg.mat');
               A=exist(filename,'file');
            if A==0,
               filenamecode=3;
            else
    ...
    for event=1:length(cfg.epochMEG_event_order),
      for  iepochMEG_filter=1:length(cfg.epochMEG_filt.hpf)
        if isfield(cfg,'epochMEG_merge_events'),
            cfgsheet.output_epoch_fields{chartcounter}=strcat('good_epochs_merged_cond_',num2str(event),'_',num2str(cfg.epochMEG_filt.hpf(iepochMEG_filter)),'-',num2str(cfg.epochMEG_filt.lpf(iepochMEG_filter)));
            cfgsheet.output_epoch_fields_indbad_EOG{chartcounter}=strcat('bad_epochs_EOG_merged_cond_',num2str(event),'_',num2str(cfg.epochMEG_filt.hpf(iepochMEG_filter)),'-',num2str(cfg.epochMEG_filt.lpf(iepochMEG_filter)));
            cfgsheet.output_epoch_fields_indbad_MEG{chartcounter}=strcat('bad_epochs_MEG_merged_cond_',num2str(event),'_',num2str(cfg.epochMEG_filt.hpf(iepochMEG_filter)),'-',num2str(cfg.epochMEG_filt.lpf(iepochMEG_filter)));
   
        else
            cfgsheet.output_epoch_fields{chartcounter}=strcat('good_epochs_cond_',num2str(event),'_',num2str(cfg.epochMEG_filt.hpf(iepochMEG_filter)),'-',num2str(cfg.epochMEG_filt.lpf(iepochMEG_filter)));
            cfgsheet.output_epoch_fields_indbad_EOG{chartcounter}=strcat('bad_epochs_EOG_cond_',num2str(event),'_',num2str(cfg.epochMEG_filt.hpf(iepochMEG_filter)),'-',num2str(cfg.epochMEG_filt.lpf(iepochMEG_filter)));
            cfgsheet.output_epoch_fields_indbad_MEG{chartcounter}=strcat('bad_epochs_MEG_cond_',num2str(event),'_',num2str(cfg.epochMEG_filt.hpf(iepochMEG_filter)),'-',num2str(cfg.epochMEG_filt.lpf(iepochMEG_filter)));
...
load(filename);
    ...
    cfgsheet.output_fields{spreadsheet_start,12}=newrun;             
    ...
    filename=strcat(subj,'_do_sss_bad_channels_cfg');
        CHECK=strcat(filename,'.mat');
        CHECKVALUE=exist(CHECK);
        if CHECKVALUE==2,
             load(filename);
...
%     if filenamecode>=4 
%       
%         if ~isfield(cfg,'epoching_folder')
%           cfg.epoching_folder='epochMEG';
%         end
%         
%         filename=strcat(masterroot,'/',cfg.epoching_folder);
%         cd(filename);
%         command=['ls ' subj '*'];
%         [st,w] = unix(command);
%         k = findstr(w, '.mat');
...
%            if st~=0
%        
%               fprintf(1,'\n Warning, Command for subject COULD NOT BE executed: %s \n',command);
%            else
%                start=1;
%                for loop=1:length(k)
%                finish=k(loop)+3;    
...
%               cfg=rmfield(cfg,'epochMEG_filt');
%               cfg.epochMEG_filt.hpf=.1;cfg.epochMEG_filt.lpf=144;
                for iepochMEG_filter=1:length(cfg.epochMEG_filt.hpf)
                        try
                            epoch_meg=[masterroot,'/epochMEG'];
                            cd(epoch_meg);
    
                                epoched_filename=(strcat(masterroot,'/','epochMEG','/',subj,'_',cfg.protocol,'_VISIT_',num2str(visitNo),'_cond_',num2str(event), '_',num2str(cfg.epochMEG_filt.hpf(iepochMEG_filter)),'-',num2str(cfg.epochMEG_filt.lpf(iepochMEG_filter)),'fil_epochs.mat'));
