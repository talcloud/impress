function [cfg]= do_sss_decimation_combined_mne(subj,visitNo,run,erm_run,cfg) 
...
diary on
if ~isfield(cfg,'erm_decimation_mne'),
cd(data_subjdir)
    %     cd(erm_subjdir);
    ...
    for irun=1:erm_run,   
                ...
                if (cfg.erm_sss),
                    ...
                    if cfg.removeECG_EOG==2,
                       cfg.erm_sss_file_tag=[subj,'_erm_',num2str(irun),'_sss_ecgeogClean_applied_raw.fif'];
                    %                 cfg.erm_sss_file_tag=[subj,'_erm_',num2str(irun),'_sss.fif'];
                tag='sss';
                fprintf('Starting to Decimate SSS File, ecgeogclean %s\n',subj);
                
                    elseif cfg.removeECG_EOG==1,
                       cfg.erm_sss_file_tag=[subj,'_erm_',num2str(irun),'_sss_ecgClean_applied_raw.fif'];
                 tag='sss';
                fprintf('Starting to Decimate SSS File, ecgClean %s\n',subj);
