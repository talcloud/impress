function [cfg]=do_calc_inverse(subj,visitNo,run,cfg)
  %% Error Check

if isfield(cfg,'current')
    I=strmatch(cfg.current,'do_calc_inverse');
           if isempty(I)
               return
           else
...
if ~isfield (cfg,'forward_operator_used_tag')
    for irun=1:run,
        ...
        for icov_tag=1:length(cfg.inv_cov_tag.hpf)
                ...
                cfg.inv_tag_loose=[' --inv '  subj '_' cfg.protocol '_',num2str(cfg.inv_cov_tag.hpf(icov_tag)),'_',num2str(cfg.inv_cov_tag.lpf(icov_tag)),'_','fil_loose_new_erm_proj-inv.fif  ' ];

            
                    
                        command=['mne_do_inverse_operator --meg --depth --loose 0.3  --megreg 0.05 --fwd ' ,cfg.forward_operator_used_tag,  cfg.inv_proj_tag cfg.erm_cov_tag cfg.inv_tag_loose, ' -v >& calc-inverse_loose_new_erm.log' ]; 
                        [st,w] = unix(command);
                        fprintf(1,'\n Command executed: %s \n',command);
                        ...
                        cfg.inv_tag_fixed = ['--inv '  subj '_' cfg.protocol '_',num2str(cfg.inv_cov_tag.hpf(icov_tag)),'_',num2str(cfg.inv_cov_tag.lpf(icov_tag)),'_','fil_fixed_new_erm_proj-inv.fif  ' ];
                        %command=['mne_do_inverse_operator --meg --fixed  --megreg 0.05  --fwd ' cfg.forward_fif{run}  cfg.inv_proj_tag{run} cfg.erm_cov_tag  '--srccov ' cfg.inv_tag_loose  cfg.inv_tag_fixed]; 
                        command=['mne_do_inverse_operator --meg --fixed  --megreg 0.05  --fwd ',cfg.forward_operator_used_tag,  cfg.inv_proj_tag cfg.erm_cov_tag  cfg.inv_tag_fixed,' -v >& calc-inverse_fixed_new_erm.log' ];
                        [st,w] = unix(command);
                        fprintf(1,'\n Command executed: %s \n',command);
