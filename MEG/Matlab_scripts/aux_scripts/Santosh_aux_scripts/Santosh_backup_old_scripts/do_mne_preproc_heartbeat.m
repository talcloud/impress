...
if isfield(cfg,'current')
    I=strmatch(cfg.current,'do_mne_preproc_heartbeat');
           if isempty(I)
               return
           else
...
%                                 cfg.projectfile= strcat(subj,'_',cfg.protocol,'_',num2str(irun),'_proj','.fif');
                                ...
                                [cfg]=clean_ecg(cfg.in_fif{irun},cfg.out_fif{irun},cfg.eventFileName,cfg.in_path{irun},cfg,visitNo,subj); % check channel STI014
                                error_handler=3;
                        catch
                 ...
                 elseif cfg.removeECG_EOG==2
                          ...
                          try   
                                ...
                                [cfg]=clean_ecgeog(cfg.in_fif{irun},cfg.out_fif{irun},cfg.projectfile,cfg.eventFileName,cfg.in_path{irun},cfg,visitNo,subj); % check channel STI014
                                error_handler=3;
                          catch
