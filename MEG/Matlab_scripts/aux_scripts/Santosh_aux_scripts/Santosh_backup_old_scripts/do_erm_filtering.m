...
if isfield(cfg,'current')
    I=strmatch(cfg.current,'do_erm_filtering');
           if isempty(I)
               return
           else
...
%% Filtering files
 ...
 for irun=1:erm_run,
   ...
   for ifilter=1:length(cfg.filt.hpf),
            ...
            '--projoff ' ' --save ' subj '_erm_' num2str(irun),'_',num2str(cfg.filt.hpf(ifilter)),'-',num2str(cfg.filt.lpf(ifilter)),'fil.fif  >& ',subj,'_',cfg.protocol,'_erm_filtering.log '];
