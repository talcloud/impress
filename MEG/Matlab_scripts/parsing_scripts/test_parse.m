function [Center Decimation Bad_channels]=parse_mne_sss_log_sss_onfif_SK(logfile)
...
% Comparing it with skipped tag
    ...
    continue;
     ...
     I = strmatch(' Bad channels for this run are:', dline);
    
    if ~isempty(I)
    Bad_channels=sscanf(dline, '%*s %*s %*s %*s %*s %*s  %d', [1, inf]);
    I=[];
    continue;
    end
